...
function btnEvent(){
    ...
    $('#search_submit').click(function(){
        var null_status = 0;
        var query = [];
        query.push(getBaseInfoObj('syrk'));
        query.push(getBaseInfoObj('syrk_relation'));
    ...
    $('#table_content').on('click','.details',function(){
        //console.log(1,$(this).attr('ryid'));
        alert('人员id：'+$(this).attr('ryid'));
    });
    //创建查询条件输入框
    createSearchInput('syrk');
    createSearchInput('syrk_relation');
    //初始化查询结果表格
    creatDatagrid();
}
...
function getBaseInfoObj(type){
    ...
    //基本信息
    if(type == 'syrk'){
        query_obj.type = 'syrk_syrkxxzb';
    //关联信息
    }else if(type == 'syrk_relation'){
        query_obj.type = 'ry_ryjzgjxxb';
    }//继续扩展...
...
function ajaxQuery(condition_obj){
    ...
    $.ajax({
        url : "bigdata/v1/syrk/s?q="+condition,
        type : 'get',
        dataType : 'json',
